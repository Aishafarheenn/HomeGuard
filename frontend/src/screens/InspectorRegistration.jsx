...
import { Shield, User, Mail, Phone, Lock, AlertCircle, Eye, EyeOff, UserCog } from 'lucide-react'
...
function InspectorRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await inspectorServices.registerInspector({
        full_name: formData.full_name,
        email: formData.email,
        phone: formData.phone,
        password: formData.password,
      })
      navigate('/login', { state: { registeredInspector: true } })
    } catch (err) {
    ...
    <div className="min-h-screen flex bg-[#F5F3FF]">
      ...
      <div className="hidden lg:flex lg:w-[45%] min-h-screen bg-gradient-to-br from-[#E6E6FA] via-[#DDD6FE] to-[#C4B5FD] flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="w-40 h-40 rounded-3xl bg-white/90 shadow-xl flex items-center justify-center mb-8">
            <UserCog className="w-20 h-20 text-[#7C3AED]" strokeWidth={1.5} />
          </div>
          <h2 className="text-2xl font-bold text-[#4C1D95] mb-2">Register as Inspector</h2>
          <p className="text-[#6D28D9]/80 max-w-xs">
            Apply to join HomeGuard. An admin will approve your account before you can sign in.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 overflow-y-auto">
        <div className="w-full max-w-[420px] bg-white rounded-3xl shadow-lg shadow-slate-200/50 p-8 sm:p-10 my-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-[#1F2937] mb-2">Inspector registration</h1>
          <p className="text-slate-500 text-sm mb-6">Apply to become an inspector. Admin approval required to sign in.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? 'Submitting…' : 'Apply as inspector'}
            </button>
          </form>
          ...
          <p className="text-center mt-2">
            <Link to="/register/owner" className="text-sm text-[#7C3AED] hover:underline">
