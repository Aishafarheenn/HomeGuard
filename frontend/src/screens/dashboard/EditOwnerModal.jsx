...
function EditOwnerModal({ isOpen, onClose, onSuccess, ownerId }) {
  ...
  useEffect(() => {
    if (isOpen && ownerId) {
      ownerServices.getOwnerById(ownerId).then((o) => {
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await ownerServices.updateOwner(ownerId, {
      ...
      const detail = err.response?.data?.detail ?? err.message ?? 'Failed to update owner'
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-[#1F2937]">Edit owner</h2>
          </div>
